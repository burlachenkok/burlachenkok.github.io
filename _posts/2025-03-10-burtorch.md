...
The new paper by [Konstantin Burlachenko](https://burlachenkok.github.io/) and [Peter Richtárik](https://richtarik.org/), titled [BurTorch: Revisiting Training from First Principles by Coupling Autodiff, Math Optimization, and Systems](https://arxiv.org/abs/2405.14852), presents the design choices behind one of the fastest and most memory-efficient backpropagation implementations on CPU.
...
BurTorch, like any high-performance framework with low-latency capabilities, carries the potential for misuse if it falls into the wrong hands. 
The authors need time to consider risks before proceeding with the distribution of the source code in any form.
